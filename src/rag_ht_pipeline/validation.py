...
from .stage5_search_ready import add_retrieval_hashes
...
def _quality_metrics(config: PipelineConfig, rows: int) -> dict[str, float | int]:
    ...
    if category.get("input_rows"):
        metrics["category_resolution_ratio"] = float(
            category.get("resolved_rows", 0)
        ) / int(category["input_rows"])
    if location.get("input_rows"):
        denominator = int(location["input_rows"])
        metrics["city_resolution_ratio"] = (
            float(location.get("resolved_city", 0)) / denominator
        )
        metrics["locality_resolution_ratio"] = (
            float(location.get("resolved_locality", 0)) / denominator
        )
    if attributes.get("input_rows"):
        metrics["attribute_mapping_ratio"] = float(
            attributes.get("mapped_ads", 0)
        ) / int(attributes["input_rows"])
        metrics["attribute_schema_mismatch_rows"] = int(
            attributes.get("schema_mismatch_rows", 0)
        )
    return metrics


def _quality_failures(
    config: PipelineConfig, metrics: dict[str, float | int]
) -> list[str]:
    settings = config.quality
    failures: list[str] = []
    minimums = {
        "category_resolution_ratio": float(
            settings.get("min_category_resolution_ratio", 0.0)
        ),
        "city_resolution_ratio": float(settings.get("min_city_resolution_ratio", 0.0)),
        "locality_resolution_ratio": float(
            settings.get("min_locality_resolution_ratio", 0.0)
        ),
        "attribute_mapping_ratio": float(
            settings.get("min_attribute_mapping_ratio", 0.0)
        ),
    }
    for metric, minimum in minimums.items():
        if metric in metrics and float(metrics[metric]) < minimum:
            failures.append(
                f"{metric}={float(metrics[metric]):.4f} is below minimum {minimum:.4f}"
            )
...
def run_final_verification(
    ...
    *,
    ...
    validation_columns = source.schema.names
    ...
    for batch_number, batch in enumerate(
        ...
        expected_hashes = add_retrieval_hashes(
            frame.drop(columns=["embedding_content_hash", "retrieval_metadata_hash"])
        )
        embedding_hash_mismatches += int(
            frame["embedding_content_hash"]
            .map(clean)
            .ne(expected_hashes["embedding_content_hash"])
            .sum()
        )
        metadata_hash_mismatches += int(
            frame["retrieval_metadata_hash"]
            .map(clean)
            .ne(expected_hashes["retrieval_metadata_hash"])
            .sum()
        )
        LOGGER.info(
    ...
    status = (
        ...
        and empty_bm25 == 0
        and wrong_company == 0
        and embedding_hash_mismatches == 0
        and metadata_hash_mismatches == 0
    ...
    report = {
        ...
        "embedding_content_hash_mismatch_rows": embedding_hash_mismatches,
        "retrieval_metadata_hash_mismatch_rows": metadata_hash_mismatches,
        "batch_size": VALIDATION_BATCH_SIZE,
